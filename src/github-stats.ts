...
type RepoLanugages = Language[];

enum LANGUAGE_TO_COLOR {
...
interface JsonCacheOptionsMap {
    repos: Repo;
    repoLanguages: RepoLanugages;
}

type JsonCacheType = keyof JsonCacheOptionsMap;

type Language = keyof LANGUAGE_TO_COLOR;

type LanguageCountMap = { [key in Language]?: number };

export async function GithubStats(username: string): Promise<GithubStats> {
    ...
    function getJSONCache(key: JsonCacheType): JsonCacheOptionsMap[JsonCacheType] {
        const value = getCache(key);
        return value ? JSON.parse(value) : null;
    }

    // TODO: typesage key-value pairs
    function saveJSONCache(key: JsonCacheType, value: JsonCacheOptionsMap[JsonCacheType]): void {
    ...
    // TODO: languagecounts interface
    function countBytesWrittenInLanguage(): {[key in Language]?: number} {
        const languageCountMap: LanguageCountMap = {};
        repoLanguages.forEach((repoLanguage: Language) =>
    ...
    const calendarSvg = doc.querySelector('.js-calendar-graph-svg');
    const days = calendarSvg.querySelectorAll('g > g > rect');
    ...
    const commitsByDay = Array.from(days).map(day => {
        return ({
            numCommits: day.dataset.count,
            date: day.dataset.date
        })
    ...
    const repos = !isUsernameChanged(username) && cachedRepos ?
        cachedRepos :
        await getJSON(`${GITHUB_API_URL}/users/${username}/repos?per_page=100`);

    let percentages;
    let repoLanguages: Language[];
    ...
    if (repos) {
        ...
        if (repoLanguages) {
            repoLanguages = repoLanguages.filter((repoLanguage: Language) => repoLanguage);
            ...
            sortedLanguages = Object.keys(languageCounts).sort((languageA: Language, languageB: Language) => languageCounts[languageB] - languageCounts[languageA]);
        ...
        languagesContribSVG: (config = {}) => {
            ...
            if (sortedLanguages) {
                ...
                for (let i = 0; i < sortedLanguages.length; i++) {
                    ...
                    const rect = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
                    const top = i * (barHeight + lineSpacing);
